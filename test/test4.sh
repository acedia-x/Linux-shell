...
service_name=$(echo "$service_name" | tr -d '\000-\037')
...
if [ "$is_running" = true ]; then
    ...
    if [ -n "$pid" ] && [ "$pid" -ne 0 ]; then
        ...
        if [ -n "$start_time" ]; then
            echo "服务名称：$service_name, PID: $pid, 端口：$port, 启动时间：$start_time"
        else
            echo "服务名称：$service_name, PID: $pid, 端口：$port, 启动时间：未知"
        fi
    else
        echo "服务名称：$service_name 正在运行，但无法获取PID"
    fi
else
    echo "服务名称：$service_name 不在运行"
fi
